...
create or replace function proxy_listener (listener_id int)
    ...
    as $function$
begin
    execute format('create view %I as %s', 'proxy_listener_' || listener_id::text, $def$
        select
            omni_httpd.urlpattern ('/*?') as match, '@extschema@.proxy_handler(omni_httpd.http_request)'::regprocedure as handler;
    $def$);
end;
$function$;
