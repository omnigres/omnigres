...
declare
    ...
    route_rec record;
begin
    for rec in select * from available_routers
        loop
            router := rec.router_relation;
            matcher := 'urlpattern';
            for route_rec in execute format('select to_json(%1$I) as row from %1$I', router)
                loop
                    declare
                        keys text[];
                    ...
                    begin
                        ...
                        with function_details as (select unnest(p.proallargtypes) as arg_type,
                        ...
                        from function_details fd
                                 join pg_type t on fd.
                                                       arg_type = t.oid
                        where t.typname = 'http_outcome'
                          and t.typnamespace = 'omni_httpd'::regnamespace;
                        return next;
                    end;
                end loop;
        end loop;
