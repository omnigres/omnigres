  ...
  for (int i = 0; i < nvals; i++) {
    ...
    if (SPI_OK_SELECT == SPI_execute_plan(entry->plan, NULL, NULL, true, 0)) {
      ...
      if (routers[NumRouters].priority_index > 0 &&
          SPI_gettypeid(SPI_tuptable->tupdesc, routers[NumRouters].priority_index + 1) != INT4OID) {
        SPI_finish();
        continue;
      }
...
#undef heap_match
  ...
  qsort(routers, NumRoutes, sizeof(struct Route), RouteCompare);
cleanup:
  SPI_finish();
  return result;
}
