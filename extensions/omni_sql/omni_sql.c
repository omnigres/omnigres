...
Datum execute_parameterized(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    if (list_length(stmts) != 1) {
      ereport(ERROR, errmsg("statement should contain only a single statement"));
    }

    struct ctx *call_ctx = palloc(sizeof(*call_ctx));
    call_ctx->stmt = cstatement;
    call_ctx->done = false;
    call_ctx->returning = omni_sql_is_returning_statement(stmts);
    ...
    if (PG_ARGISNULL(1)) {
      ...
      // Extract types, values and nullability from the parameters array
      Jsonb *parameters = PG_GETARG_JSONB_P(1);
      ...
      // Check types
      if (!PG_ARGISNULL(2)) {
        ArrayType *types = PG_GETARG_ARRAYTYPE_P(2);
  ...
  struct ctx *call_ctx = (struct ctx *)funcctx->user_fctx;
  if (call_ctx->done) {
