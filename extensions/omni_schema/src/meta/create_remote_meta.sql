...
--## for view in meta_views
            ...
            begin
                ...
                where
                 ...
                 execute format($sql$create function %1$I() returns table(%4$s)
                                begin atomic select * from public.dblink(%2$L, 'select * from %3$I.%1$I')
                                  t(%4$s);
                                end;
                           $sql$, '/*{{ view }}*/', server, remote_schema, signature);

                execute format('create view %1$I as select * from %1$I()', '/*{{ view }}*/');
            end;
--## endfor
