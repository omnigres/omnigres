...
begin
    ...
    for rec in
        select
            ...
            processor_extension
        from
            omni_vfs.list_recursively(fs, path) files
        ...
        loop
            ...
            if rec.language = 'sql' then
                ...
                for current_statement in select
                                             source
                                         from
                                             omni_sql.raw_statements(convert_from(omni_vfs.read(fs, rec.name), 'UTF-8')::cstring)
                    loop
                        insert
                        into
                            omni_schema_execution_status(filepath, language, code)
                ...
                insert
                into
                    omni_schema_execution_status(filepath, language, extension, file_processor,
                                                 file_processor_extension, processor, processor_extension, code)
                values
                    (rec.name, rec.language, rec.extension, rec.file_processor, rec.file_processor_extension,
        ...
        loop
            ...
            for current_filename in select distinct filepath from omni_schema_execution_status order by filepath
                loop
                    ...
                    for rec in select *
                               from
                                   omni_schema_execution_status
                               where
                                   execution_successful = false and
                        ...
                        loop
                            begin
                                if rec.language is null and rec.processor is not null then
                                    if not exists(select *
                                                  from
                                                      dblink(rec.filepath,
                                                             format('select true from pg_extension where extname = %L',
                                ...
                                set
                                    execution_number     = successful_statement_executions,
                            ...
                            exception
                                when others then
                                    ...
                                    update omni_schema_execution_status
                                    set
                                        last_execution_error = error_message
                                    where
                                        id = rec.id;
