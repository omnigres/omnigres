...
begin
    ...
    perform dblink(revision_conninfo,
                 format('create table omni_schema.deployed_revision as select %L as revision',
                        revision::text));
    ...
    begin
        foreach current_parent in array revision_parents
            ...
            ---- so that services know it is not an operational database
                ...
                ---- Install omni_schema in the target schema to get its meta
                perform dblink(parent_conninfo, format('create extension omni_schema version %L cascade',
                                                       (select extversion from pg_extension where extname = 'omni_schema')));

                parent_sources =
                        omni_yaml.to_json(convert_from(
                                omni_vfs.read(fs, revisions_path || '/' || current_parent || '/sources.yaml'), 'utf8'));

                for rec in with
                               parent_fs as materialized (select omni_vfs.table_fs(current_parent::text) parent_fs)
                           select
                               parent_fs.parent_fs,
                               key,
                           ...
                           from
                               parent_fs
                               inner join (select * from jsonb_each(parent_sources) as t(key, value)) files on true
                    loop
                        perform omni_vfs.write(rec.parent_fs, rec.key, rec.decoded, create_file => true);
                    end loop;

                perform
                from
                    assemble_schema(parent_conninfo, omni_vfs.table_fs(current_parent::text), '')
                where
    ...
    if not has_parents then
        ...
        perform set_config('search_path', (revision::text || '_diff')::regnamespace || ',' || old_search_path, true);
...
--## endfor
    ...
    for rec in
        with
            tables as materialized (select schema_name, name from "table" t natural inner join table_permanent tp),
    ...
    perform omni_vfs.write(fs, revisions_path || '/' || name, coalesce(omni_vfs.read(transactional_fs, name), ''),
                           create_file => true)
    from
