...
begin
    ...
    if cardinality(parents) = 0 then
        ...
        begin
            ...
            if omni_vfs.file_info(fs, migration_path) is not null then
                ...
                for rec in select * from omni_sql.raw_statements(migration::cstring)
                    loop
                        begin
    ...
    ---- Create a foreign server to connect to the revision
    ---- Because postgres_fdw enforces repeatable read, we need a new connection
    execute format(
            $sql$create server %1$I foreign data wrapper postgres_fdw options(host %2$L, dbname %4$L, port %3$L)$sql$,
            source_db || '_1', current_setting('unix_socket_directories'), current_setting('port'), source_db);
    execute format('create user mapping for %1$I server %2$I options (user %1$L)',
                   current_user, source_db || '_1');

    ---- Define remote meta
    execute format('create schema %I', target);
    perform create_remote_meta(target::text::regnamespace, 'omni_schema'::name, (source_db || '_1')::name,
                               materialize => true);
            ...
            loop
                ...
                if omni_vfs.file_info(fs, diff_path) is not null then
                    ...
                    perform set_config('search_path',
                                       diff_schema || ',' || old_search_path, true);

--## for direction in ["added","removed"]
--## for view in meta_views
                    expected_diff := jsonb_strip_nulls(jsonb_set(expected_diff, '{/*{{ direction }}*/_/*{{ view }}*/}',
                                                                 coalesce((select
                    ...
                    if expected_diff != diff then
                        ...
                        from
                            (select
                             ...
                             where
                                 (a_value is not null and b_value is not null and a_value is distinct from b_value) or
                                 a_value is null or
                                 b_value is null) t;
