...
as
$$
select string_agg(format('%1$I => $%2$s :: %3$I', name, idx, t.typname), ', ' order by idx)
from pg_proc p,
...
as
$$
select jsonb_agg(passed_values ->> (name::text)
                 order by idx)
from pg_proc p,
     unnest(proargnames, proargtypes, proargmodes)
         with ordinality as _ (name, type, mode, idx)
...
create procedure postgrest_rpc(request omni_httpd.http_request, outcome inout omni_httpd.http_outcome,
...
begin
    ...
    arguments_definition :=
            coalesce(omni_rest._postgrest_function_call_arguments(function_reference, passed_arguments), '');
    if (select proretset from pg_proc where oid = function_reference) then
        query := format('select jsonb_agg(row_to_json(r.*)::jsonb) as result from %1$s(%2$s) r', function_reference,
                        arguments_definition);
    else
    ...
    if request.method = 'GET' then
        ...
        select (select jsonb_object_agg(keys.key, values.value)
                from unnest(omni_web.parse_query_string(request.query_string))
                         with ordinality as keys (key, ikey)
                         join unnest(omni_web.parse_query_string(request.query_string))
                    with ordinality as values (value,
                                               ivalue) on ikey % 2 = 1
                    and ivalue % 2 = 0
                    and ikey = ivalue - 1)
        into argument_values;
    else
        select (select convert_from(request.body, 'utf-8')::jsonb)
        into argument_values;
    end if;
    -- Run it
    declare
        ...
        hint    text;
    begin
        select omni_sql.execute(query, coalesce(
                omni_rest._postgrest_function_ordered_argument_values(function_reference, passed_arguments,
                                                                      argument_values), '[]'::jsonb)) -> 'result'
        into result;
        if jsonb_typeof(result) = 'array' then
            outcome := omni_httpd.http_response(result, headers => array [omni_http.http_header('Content-Range', '0-' ||
