...
as
$$
select string_agg(format('%1$I = $%2$s :: %3$I', a.attname, pa.idx, t.typname), ', ' order by pa.idx)
from pg_attribute a
...
as
$$
select jsonb_agg(passed_values ->> (name::text) order by idx)
from unnest(field_names) with ordinality as _ (name, idx);
$$;


create procedure postgrest_update(request omni_httpd.http_request, outcome inout omni_httpd.http_outcome,
...
begin
    ...
    for preference in
        ...
        loop
            ...
            begin
                case
                    when preference_name = 'missing' then _missing := preference_value;
                    when preference_name = 'return' then _return := preference_value;
                    when preference_name = 'tx' then _tx := preference_value;
                    end case;
            end;
        end loop;
    declare
    ...
    begin
        ...
        query :=
                format('update %1$I.%2$I set %3$s where %4$s %5$s',
                       ...
                       case
                           when _return = 'representation' then
        ...
        outcome :=
                omni_httpd.http_response(status => 201, body => case when _return = 'representation' then result end);
    end;
end;
$$;
