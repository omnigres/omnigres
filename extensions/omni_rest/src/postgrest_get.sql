...
begin
    ...
    if input !~ '[()]' then
        select jsonb_agg(jsonb_build_object('operator', omni_rest.postgrest_convert_operator(split_part(term, '.', 2)),
                                            'operands',
            ...
            if token = '(' then
                ...
                if depth = 0 then
                    result := result || case lower(current_production)
                                            ...
                                            when 'or' then
                                                jsonb_build_object('operator', 'or', 'operands',
                                                                   omni_rest.postgrest_parse_logical(current))
                                            when 'and' then
                                                jsonb_build_object('operator', 'and', 'operands',
                                                                   omni_rest.postgrest_parse_logical(current))
                                            when 'not.or' then
                                                jsonb_build_object('operator', 'not', 'operands',
                                                                   jsonb_build_array(jsonb_build_object('operator',
                                                                                                        'or',
                                                                                                        'operands',
                                                                                                        omni_rest.postgrest_parse_logical(current))))
                                            when 'not.and' then
            ...
            if depth = 0 then
                if token = ',' then
                    if current_production !~ '[()]' then
                        result := result || jsonb_build_array(jsonb_build_object('operator',
                                                                                 omni_rest.postgrest_convert_operator(split_part(current_production, '.', 2)),
                                                                                 'operands', jsonb_build_array(
                                                                                         split_part(current_production, '.', 1),
                                                                                         split_part(current_production, '.', 3))));
                    end if;
                    current_production := '';
                else
...
as
$$
with indexed_params as (select *
                        from unnest(params)
                                 with ordinality r (param, index)),
     comparisons as (select lower(columns.param)                                                   as key,
                     ...
                     from indexed_params columns
                              join indexed_params values
                                   on columns.index % 2 = 1
                                       and values.index % 2 = 0
                                       and columns.index = values.index - 1)
select jsonb_build_object('operator', 'and', 'operands', coalesce(jsonb_agg(
                                                                          case
                                                                              when key = 'not.and' then
                                                                                  jsonb_build_object('operator', 'not',
                                                                                                     'operands',
                                                                                                     jsonb_build_array(jsonb_build_object(
                                                                                                             'operator',
                                                                                                             'and',
                                                                                                             'operands',
                                                                                                             omni_rest.postgrest_parse_logical(value))))
                                                                              when key = 'and' then
                                                                              ...
                                                                              when key = 'not.or' then
                                                                                  jsonb_build_object('operator', 'not',
                                                                                                     ...
                                                                                                     jsonb_build_array(jsonb_build_object(
                                                                                                             ...
                                                                                                             omni_rest.postgrest_parse_logical(value))))
                                                                              when key = 'or' then
                                                                                  jsonb_build_object('operator', 'or',
                                                                                                     'operands',
                                                                                                     omni_rest.postgrest_parse_logical(value))
                                                                              when first_operator = 'not' then
                                                                                  jsonb_build_object('operator', 'not',
                                                                                                     ...
                                                                                                     jsonb_build_array(jsonb_build_object(
                                                                                                             ...
                                                                                                             second_operator,
                                                                                                             'operands',
                                                                                                             jsonb_build_array(key, split_part(value, '.', 3)))))
                                                                              when first_operator is not null then
                                                                                  jsonb_build_object('operator',
                                                                                                     first_operator,
                                                                                                     'operands',
                                                                                                     jsonb_build_array(key, (regexp_match(value, '\.(.*)'))[1]))
                                                                              else
                                                                                  jsonb_build_object('operator',
                                                                                                     'invalid_operator',
                                                                                                     'operands',
...
declare
    output text;
begin
    select case where_ast ->> 'operator'
               when 'invalid_operator' then
                   'invalid_operator invalid_operator'
               when 'not' then
                   format('not (%1$s)', omni_rest.postgrest_format_get_param(where_ast -> 'operands' -> 0))
               when 'and' then
                   (select string_agg(omni_rest.postgrest_format_get_param(op), ' and ')
                    from jsonb_array_elements(where_ast -> 'operands') op)
               when 'or' then
                   (select string_agg(omni_rest.postgrest_format_get_param(op), ' or ')
                    from jsonb_array_elements(where_ast -> 'operands') op)
               else
                   format('%1$I %2$s %3$L', where_ast -> 'operands' ->> 0, where_ast ->> 'operator',
...
begin
    ...
    if query is null then
        ...
        into _order;
        -- Finalize the query
        query := format('select %3$s from %1$I.%2$I where %4$s %5$s', namespace, (select relname
                                                                                  from pg_class
                                                                                  where oid = relation),
                        concat_ws(', ', variadic query_columns), coalesce(_where, 'true'), _order);
    ...
    begin
        ...
        outcome := omni_httpd.http_response(result, headers => array [omni_http.http_header('Content-Range',
                                                                                            _offset || '-' ||
    ...
    exception
        when others then
            ...
            outcome := omni_httpd.http_response(jsonb_build_object('message', message, 'detail', detail, 'hint', hint),
                                                status => 400);
    end;
