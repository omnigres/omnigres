...
as
$$
with arguments as (select oid,
                          array_agg(coalesce(name, ''))
                          filter (where idx <= (pronargs - pronargdefaults)) as required_parameters,
                   ...
                   having count(*) - count(name) = 0)
select array_agg(p.oid::regproc)
from pg_proc p
...
create procedure _postgrest_function(request omni_httpd.http_request, function_reference inout regproc,
                                     namespace inout text, settings postgrest_settings default postgrest_settings())
    language plpgsql
as
$$
declare
...
begin
    ...
    all_references :=
            omni_rest._postgrest_function_by_arguments(namespace, function_name, coalesce(function_arguments, '{}'));
    select all_references[1]
    into function_reference;
