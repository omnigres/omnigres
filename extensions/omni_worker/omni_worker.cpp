...
template <message T> struct omni_pool_handler {
  ...
  omni_pool_handler(const std::string &library,
...
std::string arena_name() {
  return cppgres::fmt::format("omni_pool_v1_{}_{}", cppgres::ffi_guard{::GetSystemIdentifier}(),
                              MyDatabaseId);
}
std::size_t arena_size() { return 1024 * 1024; }
std::string mq_name() {
  return cppgres::fmt::format("omni_pool_v1_{}_{}_mq", cppgres::ffi_guard{::GetSystemIdentifier}(),
                              MyDatabaseId);
}
...
postgres_function(reload_handlers, ([]() -> cppgres::value {
                    if (CALLED_AS_TRIGGER(
                      ...
                      // auto td = reinterpret_cast<TriggerData *>(
                      // cppgres::current_postgres_function::call_info().value()->context);
                      reload_upon_commit = true;

                      return cppgres::value(cppgres::nullable_datum(0), cppgres::type(TRIGGEROID));
                    }
...
static void worker(cppgres::datum main) {
  ...
  std::thread receiver([&]() {
    while (true) {
      rcvr.receive<reload>(overload{[&](reload &) {
                                        ...
                                        worker
                                            .post([&]() {
                                              reinterpret_cast<void (*)(const void *)>(
                                                  it->second.ptr)(
                                                  static_cast<char *>(arena.get_address()) +
                                                  msg.offset);
                                            })
                                            .wait();
                                      }
...
void _Omni_init(const omni_handle *handle) {
  cppgres::exception_guard([&]() {
    ...
    omni_hook txn_hook = {.name = "omni_worker transaction hook",
                          .type = omni_hook_xact_callback,
                          .fn = {.xact_callback = [](omni_hook_handle *handle, XactEvent event) {
