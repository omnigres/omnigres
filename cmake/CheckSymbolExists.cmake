# This is a forked version of CheckSymbolExists that checks which
# include files were tested for caching purposes.
# See https://gitlab.kitware.com/cmake/cmake/-/merge_requests/8042
...
macro(__CHECK_SYMBOL_EXISTS_IMPL SOURCEFILE SYMBOL FILES VARIABLE)
    unset(__CHECK_SYMBOL_EXISTS_CACHED)
    # If ${VARIABLE} has been previously defined,
    if(DEFINED "${VARIABLE}")
        # check if it was defined in one of the files we're checking against
        foreach(FILE ${FILES})
            if(FILE IN_LIST "${${VARIABLE}_FILES}")
                set(__CHECK_SYMBOL_EXISTS_CACHED TRUE)
            endif()
        endforeach()
        # (because other users may have been checking other files)
    endif()
    if(NOT DEFINED __CHECK_SYMBOL_EXISTS_CACHED OR NOT DEFINED "${VARIABLE}" OR "x${${VARIABLE}}" STREQUAL "x${VARIABLE}")
        set(_CSE_SOURCE "/* */\n")
