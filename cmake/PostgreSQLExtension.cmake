...
function(add_postgresql_extension NAME)
    ...
    if (NOT _ext_PRIVATE)
        if (NOT DEFINED _ext_VERSION)
            get_version(${NAME} _retrieved_ext_VERSION)
            message(FATAL_ERROR "Version for ${NAME} is unspecified, versions.txt suggests ${_retrieved_ext_VERSION}")
        else ()
            get_version(${NAME} _retrieved_ext_VERSION)
            if (NOT "${_retrieved_ext_VERSION}" STREQUAL "${_ext_VERSION}")
                message(STATUS "Version for ${NAME} has diverged from versions.txt")
                if ("${_ext_VERSION}" VERSION_GREATER "${_retrieved_ext_VERSION}")
                    message(STATUS "${_ext_VERSION} is greater than ${_retrieved_ext_VERSION}, updating versions.txt")
                    set_version(${NAME} ${_ext_VERSION})
                else ()
                    message(FATAL_ERROR "${_ext_VERSION} is lesser than ${_retrieved_ext_VERSION}, is it out of sync?")
                endif ()
            endif ()
        endif ()
    endif ()
