...
function(prepare_compressed_dependency dependency)
    ...
    set(_single FILE)
    set(_multi)
    cmake_parse_arguments(_dependency "${_optional}" "${_single}" "${_multi}" ${ARGN})
    if (DEFINED _dependency_FILE)
        set(_dependency_file "${_dependency_FILE}")
    endif ()

    if (NOT DEFINED _dependency_file AND EXISTS "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../deps/${dependency}.7z")
        set(_dependency_file "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../deps/${dependency}.7z")
    endif ()
    if (NOT DEFINED _dependency_file)
        message(FATAL_ERROR "Dependency ${dependency} not found")
    endif ()
    ...
    if (NOT EXISTS "${_dependency_dir}")
        ...
        if (APPLE)
            execute_process(COMMAND xattr -rd com.apple.quarantine "${_dependency_dir}")
        endif ()
    endif ()
